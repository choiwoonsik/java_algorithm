...
public class 최소힙_1927 {
    ...
    private static void add(int node) {
        ...
        makeMinHeapADD(pq.size);
    }

    private static void makeMinHeapADD(int cur) {
        if (cur == 1) return;
        int parent = cur / 2;
        if (pq.items[parent] > pq.items[cur]) {
            swap(parent, cur);
            makeMinHeapADD(cur / 2);
        }
    }
    ...
    private static void makeMinHeapPOLL(int node) {
        // 둘다 없는 경우 : 리프 노드
